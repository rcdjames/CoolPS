...
if ($ExecPolicy -ne "RemoteSigned")
{
    ...
    $ExecPolicy = Get-ExecutionPolicy
    Write-Host "DONE - Changed Execution Policy:" $ExecPolicy -ForegroundColor Green
}
Try {
    ...
    $Wshell.Popup("Windows Update operation has completed. Reboot the system now.", 0, "WU Completed", 0x0)
} Catch {
